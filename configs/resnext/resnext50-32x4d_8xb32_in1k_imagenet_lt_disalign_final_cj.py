...
model = dict(
    ...
    head=dict(
        type='DisAlignLinearClsHead',
        num_classes=1000,
        in_channels=2048,
        loss=dict(
            type='GRWLoss',
            exp_scale=1.0,
            frequency_from='data/ImageNet_LT/meta/imagenet_lt\
            _category_frequency.json'),
        topk=(1, 5)))
...
optim_wrapper = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            ...
            'head.fc.bias': dict(lr_mult=0.0, decay_mult=0.0),
            'head.fc.weight': dict(lr_mult=0.0, decay_mult=0.0)
        }))
...
log_level = 'INFO'
load_from = 'work_dirs/resnext50-32x4d_8xb32_in1k_imagenet_lt_cj/epoch_100.pth'
resume = False
