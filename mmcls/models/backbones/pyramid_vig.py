...
class pyramid_vig(torch.nn.Module):
    ...
    def __init__(self, k, dropout, use_dilation, epsilon, use_stochastic,
                 drop_path, blocks, channels, n_classes, emb_dims):
                ...
                self.backbone += [
                    Seq(
                        Grapher(
                            ...
                            drop_path=dpr[idx],
                            relative_pos=True),
                        FFN(channels[i], channels[i] * 4, drop_path=dpr[idx]))
                ]
                idx += 1
