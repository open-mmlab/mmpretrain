...
class PluginsBaseCNNBlock(BaseModule, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        if self.with_plugins:
            assert all(
                p['position'] in self.allowed_position for p in plugins
    ...
    def build_plugins(self):
        ...
        assert set(self.position2inchannels_map.keys()) == set(
...
class BaseBottleNeck(PluginsBaseCNNBlock):
    ...
    def __init__(self, in_channels, out_channels, plugins=None, init_cfg=None):
...
class BaseBasicBlock(PluginsBaseCNNBlock):
    ...
    def get_position2inchannels_map(self):
        """get the input channels in different place."""
        return dict(
            after_conv1=self.mid_channels, after_conv2=self.out_channels)
