...
class MultiTaskClsHead(ClsHead):
    """Multi task head.
    Args:
        heads (dict): Sub heads to use, the key will be use to rename the loss components
    ...
    def simple_test(self, x, post_process=True, **kwargs):
        ...
        Returns:
            list[tuple]: The inference results.
                the output is a list, one item per batch elemnt, each item contains a tuple, containing the results of each head
        """
        assert post_process == True, "post_process=False is not implemented for MultiTaskClsHead.simple_test"
        return list(zip(*[head.simple_test(x, post_process=post_process, **kwargs) for head in self.heads]))