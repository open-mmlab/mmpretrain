...
def accuracy_numpy(pred, target, topk=1, thr=None):
    res = []
    maxk = max(topk)
    num = pred.shape[0]
    ...
    for k in topk:
        ...
        if thr is not None:
            ...
            correct_k = correct_k & (pred_score[:, :k] > thr)
        correct_k = np.logical_or.reduce(correct_k, axis=1)
        res.append(correct_k.sum() * 100. / num)
    return res


def accuracy_torch(pred, target, topk=1, thr=None):
    res = []
    maxk = max(topk)
    num = pred.size(0)
...
def accuracy(pred, target, topk=1, thr=None):
    ...
    Args:
        ...
        thr (float, optional): If not None, predictions with scores under
    ...
    if isinstance(pred, torch.Tensor) and isinstance(target, torch.Tensor):
        res = accuracy_torch(pred, target, topk, thr)
...
class Accuracy(nn.Module):
    ...
    def __init__(self, topk=(1, ), thr=None):
