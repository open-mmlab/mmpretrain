...
HOOKS = Registry(
    'hook', parent=MMENGINE_HOOKS, locations=['mmcls.engine.hooks'])

# Registries For Data and the related
# manage data-related modules
DATASETS = Registry(
    'dataset', parent=MMENGINE_DATASETS, locations=['mmcls.datasets'])
DATA_SAMPLERS = Registry(
    'data sampler',
    parent=MMENGINE_DATA_SAMPLERS,
    locations=['mmcls.datasets.samplers'])
TRANSFORMS = Registry(
    'transform',
    parent=MMENGINE_TRANSFORMS,
    locations=['mmcls.datasets.transforms'])

# manage all kinds of modules inheriting `nn.Module`
MODELS = Registry('model', parent=MMENGINE_MODELS, locations=['mmcls.models'])
...
BATCH_AUGMENTS = Registry('batch augment', locations=['mmcls.models'])
...
OPTIMIZERS = Registry(
    ...
    locations=['mmcls.engine.optimizers'])
...
METRICS = Registry(
    'metric', parent=MMENGINE_METRICS, locations=['mmcls.evaluation'])
# manage all kinds of evaluators
EVALUATORS = Registry(
    'evaluator', parent=MMENGINE_EVALUATOR, locations=['mmcls.evaluation'])

# manage task-specific modules like anchor generators and box coders
TASK_UTILS = Registry(
    'task util', parent=MMENGINE_TASK_UTILS, locations=['mmcls.models'])
