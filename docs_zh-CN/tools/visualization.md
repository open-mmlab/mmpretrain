...
- `--number`: 可视化样本数量。如果没有指定，默认为 `sys.maxint`，表示整个数据集。
...
- `--adaptive`: 是否自动调节可视化图片的大小。如果没有制定，默认为 `True`。
...
2. `--adaptive` 为 `True` 时，会自动的调整过大和过小的图片；比如 `cifra100` 中图片过小，难以写入类别信息，`--adaptive` 会自动放大图片。
