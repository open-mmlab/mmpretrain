...
import re
from .vqa import _process_punctuation, _process_digit_article
...
class ChartQARelaxACC(BaseMetric):
    ...
    def compute_metrics(self, results: List):
        ...
        for result in results:
            ...
            if is_digit:
                if self.is_digit(pred_answer):
                    ...
                    upper_bound = max(gt_answer - gt_answer * self.relax_thresh, gt_answer + gt_answer * self.relax_thresh)
                    lower_bound = min(gt_answer - gt_answer * self.relax_thresh, gt_answer + gt_answer * self.relax_thresh)
                    chart_acc = float(all([pred_answer >= lower_bound, pred_answer <= upper_bound]))
                else:
                    chart_acc = 0.0
            else:
        ...
        metrics = {'ChartQA-H acc': ChartQA_H_acc, 'ChartQA-M acc': ChartQA_M_acc, 'Overall acc': accuracy}
